...
const TodoListCreate:React.FC<Props> = ({setUpdate}) => {
    ...
    const onClickHandle = async () => {
        if (inputRef.current != null) {
            await fetch(`${process.env.API_URL}/list?email=${session?.user?.email}`, {
                method: 'POST',
    ...
    return (
        <div className='w-5/6 h-12 py-4 px-8 mt-8 bg-primary m-auto rounded-full flex align-middle justify-between relative'>
            <input ref={inputRef} type="text" name='newTodo' placeholder="Create a new list" className=' w-5/6 bg-transparent text-gray-300 font-semibold border-none outline-none text-md'/>
